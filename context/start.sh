...
# the kedro user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ] ; then
    ...
    # Refit the kedro user to the desired the user (NB_USER)
    if id kedro &> /dev/null ; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" kedro 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the kedro user:"
            _log "- username: kedro       -> ${NB_USER}"
            _log "- home dir: /home/kedro -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the kedro user or '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the kedro home directory to the desired users home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "kedro" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/kedro to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            if cp -a /home/kedro/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/kedro to /home/${NB_USER}!"
                _log "Attempting to symlink /home/kedro to /home/${NB_USER}..."
                if ln -s /home/kedro "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/kedro to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/kedro/"* ]]; then
...
# user we started as.
    ...
    KEDRO_UID="$(id -u kedro 2>/dev/null)"  # The default UID for the kedro user
    KEDRO_GID="$(id -g kedro 2>/dev/null)"  # The default GID for the kedro user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old kedro user to nayvoj ($(id -u kedro):$(id -g kedro))"
            ...
            sed --expression="s/^kedro:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/kedro:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "kedro" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/kedro. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/kedro ]]; then
        _log "WARNING: no write access to /home/kedro. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
